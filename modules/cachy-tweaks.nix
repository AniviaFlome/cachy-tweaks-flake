...
in {
  ...
  config = mkIf (cfg.enable || cfg.all) {
    ...
    services.udev.extraRules = mkIf (cfg.udevRules || cfg.all) (mkMerge [
      (mkDefault ''
        # Audio permissions
    ...
    boot.extraModprobeConfig = mkIf (cfg.modprobeConfig || cfg.all) (mkMerge [
      (mkDefault ''
    ...
    boot.kernelParams = mkIf (cfg.kernelTweaks || cfg.all) (mkMerge [
      (mkDefault [
        "max_ptes_none = 409"
      ])
    ]);

    services.journald.extraConfig = mkIf (cfg.journaldTweaks || cfg.all) (mkMerge [
      (mkDefault ''
        SystemMaxUse=50M
      '')
    ]);
